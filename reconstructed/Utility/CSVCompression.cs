    ...
    internal class CSVCompression
    {
        ...
        public static object[] Compress(string source, int deltaRows, char containerDelimiter = ';', char rowDelimiter = ',')
        {
            ...
            if (outputArr.Length > 0)
            {
                ...
                // the symbol table stage is shipped but its call site is disabled so this whole
                // dictionary compression pass is dead and the returned symbol table is always empty
            }
            List<string> outputList = new List<string>();
            foreach (string[] strArray in outputArr)
                outputList.Add(string.Join(rowDelimiter.ToString(), strArray));
            byte[] compressed = DataCompressors.Brotli.CompressString(string.Join(containerDelimiter.ToString(), outputList.ToArray()), CompressionLevel.Fastest);
        ...
        {
            string uncompressed = DataCompressors.Brotli.DecompressString(source, CompressionAlgorithm.DefaultBufferSize, CompressionAlgorithm.DefaultMaxSize);
            List<string[]> list = new List<string[]>();
        ...
        {
            string[] lastRow = new string[100]; // running previous row buffer capped at 100 columns
